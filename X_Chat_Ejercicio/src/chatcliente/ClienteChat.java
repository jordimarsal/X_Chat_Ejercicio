...
public class ClienteChat extends JFrame implements ActionListener {
	...
	// constantes
	private static final long serialVersionUID = -2797432157450644863L;
	...
	private static Integer usuariosActivos; // contador de servidor + usuarios para asignar al array users
	...
	private static Integer thisUser = 0; // se recibe el numero con el HELO (solo admite un digito, por tanto de 0 a 9,
											// aunque 0 es el servidor o no asignado al inicio)
	...
	public static void main(String[] args) {
		...
		String arg2 = "-h:127.0.0.1 -p:1234 -n:Donete";

		ip = argsCli.getIp();
		puerto = argsCli.getPuerto();
		nick = argsCli.getNick();
		ClienteChat main = new ClienteChat(); // Instanciacion de la clase ClienteChat
	...
	public void procesarEnvio(String string) {
		System.out.println("ClienteEnvio[" + nick + "/" + ClienteChat.thisUser + "]:" + ACK + thisUser + string);
		out.println(ACK + thisUser + string);
	}

	public static void enviarAdios() {
		System.out.println("ClienteDespedida[" + nick + "/" + ClienteChat.thisUser + "]:" + "BYE_" + thisUser + nick);
